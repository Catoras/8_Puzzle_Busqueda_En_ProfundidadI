...
public:
    ...
    vector<string> busquedaPorProfundidad () {
        ...
        int profundidadActual = 2;
        if (resultado->getCadena() == this->resultado_esperado) {
            cout << "Resultado Encontrado" << endl;
            return getPath(resultado);
        }
        while (true){
            while (resultado->getProfundidad() < profundidadActual) {
                if (!resultado->getEstaEstancado() || resultado->getProfundidad() < profundidadActual) {
